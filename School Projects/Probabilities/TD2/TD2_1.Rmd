...
$X(\Omega)= \mathbb{N}^*$
<br>
$X \sim Géom(p) : P(X=k) = p(1-p)^{k-1}$

2. <u>Hypothèses</u> : somme de n épreuves de Bernoulli indépendantes :
<br>
$X(\Omega)= [[0,n]]$
<br>
$X \sim Bin(n,p) : P(X=k) = \binom{n}{k} p^k(1-p)^{n-k}$
<br>

3. $E_{géo}[X] = \frac{1}{p}$ et $E_{bin}[X] = np$ 
<br><br><br>
...
1. $X(\Omega)= \mathbb{N}^*$
On cherche le 1$^{er}$ succès, c'est donc une loi géométrique : X $\sim$ Géom(p) <br>
L'espérance vaut : E[X] = $\frac{1}{p}$ = 4 

<br>

2. On intuite facilement que : <br>
$\forall i \in [[0,3]], \; \pi_i = P(X=i+1 \mid X>i) = \frac{1}{4-i}$ <br><br>
Calculons la loi de X : <br>
$P(X=1) = \frac{1}{4}$ <br>
$P(X=2) = \frac{3}{4} \times \frac{1}{3} = \frac{1}{4}$ <br>
$P(X=3) = \frac{3}{4} \times \frac{2}{3} \times \frac{1}{2} = \frac{1}{4}$ <br>
$P(X=4) = \frac{3}{4} \times \frac{2}{3} \times \frac{1}{2} \times 1 = \frac{1}{4}$ <br><br>
...
On reconnaît une loi uniforme : $U_X([[1,4]])$ <br>
On a donc $E_{unif}[X] = \frac{4+1}{5} = \frac{5}{2}$
...
$P(X=1) = \frac{1}{4}$ <br>
et que : <br>
$\rho_i = P(X>i+1 \mid X>i) = \frac{2}{3}$ <br><br>
Soit $i \ge 2 \:$ : <br>
...
$\qquad \quad \; \; = \frac{1}{4} (\frac{2}{3})^{i-2}$

<br>

4. On a déjà que : <br>
$Z(\Omega) = \mathbb{N}^*$ et $X(\Omega) = \mathbb{N}^*$ <br><br>
Et : <br>
...
<br>
Or, <br>
$P(Z=1) = P(G=0) \times P(Y=1) + P(Y=0) = \frac{1}{4}$
$P(Z=k) = P(G=k-1) \times \frac{3}{4}+ 0 = \frac{1}{4} (\frac{2}{3})^{k-2}$
<br>
Donc : X suit la même loi que Z. <br><br>
Et : 
$E[X] = E[1+Y \times G] = 1+E[Y \times G]$ <br>
$\qquad \quad \; \; = 1 + E[Y] \times E[G] \;$ par indépendance <br>
$\qquad \quad \; \; = 1 +\frac{3}{4} \times 3$ <br>
$\qquad \quad \; \; = \frac{13}{4}$
<br><br><br>
...
On a : $X \sim Géom(10^{-6})$ <br>
Pour calculer la probabilité qu'aucune fusée ne décolle au cours des $10^6$ premiers lancers : <br>
On pose $X_i$ = "la fusée ne décole pas à l'étape i" et on prend n = $10^6$, si bien que : <br>
$P(X>n) = P(\cap_{i=1}^{n} X_i) = \prod_{i=1}^{n}P(X_i) = (1-\frac{1}{n})^n \approx \frac{1}{e}$
<br><br>
<hr>
...
On pose X : le nombre de lancers des 2 dés si bien que $X \sim Géo(\frac{19}{36})$ <br>
On a donc : $E[X] = \frac{36}{19} = \frac{coût}{2}$
